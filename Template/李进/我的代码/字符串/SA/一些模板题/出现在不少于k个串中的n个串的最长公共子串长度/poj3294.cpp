...
struct SA{
	int t1[N],t2[N],cc[N],x[N],sa[N],R[N],H[N],h[N],n,m,cnt,str_cnt,num0; int *s; int num[105],home[N]; //ST T;
	...
	inline bool check_group(int l,int r) {
		//...
		count(l,r);
		if (num0>=str_cnt/2+1) return true; return false;
	}
	...
	inline int Q(){
		...
		flag=true;
		if (str_cnt==1) {
			rep(i,0,n-1) printf("%c",s[i]);
			printf("\n"); 
			return 0;
		}
		int ans=binary(0,n);
		if (!ans) return 0*printf("?\n");
		divide_group(ans,x);
		rep(i,1,cnt) if (check_group(x[i-1],x[i]-1)) {
			rep(j,sa[x[i-1]],sa[x[i-1]]+ans-1) printf("%c",s[j]);
			printf("\n");
		} return 0;
	}
