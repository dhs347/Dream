...
struct SA{
	int t1[N],t2[N],cc[N],x[N],sa[N],R[N],H[N],h[N],n,m,cnt,str_cnt,num0; int *s; int num[205],home[N],pl[N],pr[N]; //ST T;
	...
	inline bool check_group(int l,int r,int ans) {
		//...
		if (r-l+1<str_cnt/2) return false;
		count(l,r);
		if (num0<str_cnt/2) return false;
		rep(i,1,str_cnt/2) if (!num[2*i-1] && !num[2*i]) return false;
		return true;
	}
