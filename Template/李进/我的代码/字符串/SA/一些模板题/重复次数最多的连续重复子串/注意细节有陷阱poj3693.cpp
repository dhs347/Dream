...
struct SA{
	...
	inline int Q(){
		//...
		int len=n/2,ans=0,t=0,t0,t1,ans0=0,tt;
		rep(i,1,len/2) {
			for (int k=1; ; k++) {
				if (k*i>=len) break;
				if (s[(k-1)*i]!=s[k*i]) continue;
				t=Q_lcp((k-1)*i,k*i);
				t1=Q_lcp(n-1-(k-1)*i,n-1-k*i);
				t+=t1-1+i;
				int l=(t/i)*i;
				int pos=(k-1)*i-t1+1,pp=oo,qq=-oo;
				rep(j,0,t-l) if (R[pos+j]<pp) {
					pp=R[pos+j];
					qq=pos+j; 
				}
				if (t/i>ans) {
		...
		rep(i,ans0,ans0+t0-1) printf("%c",s[i]);
		return 0*printf("\n");
	}
...
int main() {
    ...
    while (scanf("%s",s)!=EOF && s[0]!='#') {
    	...
    	T.Init();
    	T.Q();
    	/*rep(i,0,T.n-1) printf("%c",T.s[i]);
    	printf("\n");
    	rep(i,0,T.n+10) printf("%c",T.s[i]);
    	printf("\n");
