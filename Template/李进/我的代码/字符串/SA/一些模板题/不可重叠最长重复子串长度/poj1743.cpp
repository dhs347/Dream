...
struct SA{
	...
	inline bool check_group(int l,int r,int ans) {
		...
		if (l==r) return false;
		int p=oo,q=-oo;
		rep(i,l,r) p=min(p,sa[i]),q=max(q,sa[i]);
		if (q-p>ans) return true; return false;
	}
	...
	inline void Insert(int _s[],int op=0,int _n=0){
		if (n) s[n++]=10000+str_cnt+1; ++str_cnt;
		if (!op) rep(i,0,_n-1) s[n++]=_s[i]; else
				 per(i,_n-1,0) s[n++]=_s[i];
	}
	//SA(){ s=new int[N]; n=str_cnt=0; }
	inline void Init(int _s[]=NULL,int _n=0,int _m=300){
