...
struct PFT{
    struct node { int y,nxt,v; } e[N<<1]; SBT T;
    int n,tot,k,rt,head[N],f[N],vis[N],sz[N]; int s[N],temp[N],flag,_x; ll ans;
    void Init(int _n,int _k){ n=_n,tot=0; mem(vis,0),mem(head,0); f[0]=oo; temp[0]=0;  /*  */ k=_k; T.init(); }
    void addedge(int x,int y,int v){ e[++tot].nxt=head[x]; head[x]=tot; e[tot].y=y; e[tot].v=v; }
