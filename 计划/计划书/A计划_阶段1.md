...
8）优化$dp$与回文切割/$k-factorization$

- 询问字符串$S$是否可以$k-$回文分解，等价于询问最小的奇数段和偶数段回文分解

- $palindromic\space length$是最小的无条件回文分解的段数，使用带$seriesLink$的$PAM$，可以轻松做到$O(nlogn)$，但这未必是最优的复杂度；注意一个$series$顶部节点的$dp$值存放的是关于一系列下标集合的$ans$值的综合，而不是直接关于节点的或者关于串的信息
