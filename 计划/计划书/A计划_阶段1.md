...
有模板题和模板：***LinearRecursion***

https://www.lydsy.com/JudgeOnline/problem.php?id=4161

简要笔记：

$O(n^2)$可以使用长除法，模拟多项式除法/取模，参见模板
...
若$x^n=F(x)G(x)+C(x)$，则有$A^n=C(A)$

更进一步地：$A^n \overrightarrow{x}=C(A)\overrightarrow{x}=C(A\overrightarrow{x})$

假设我们取具体而特定的：$\overrightarrow{x}^T=[x_{m-1},x_{m-2},x_{m-3},\dots,x_1,x_0]$

为了得到$x_n$，我们应该左乘$A_{n-m+1}$,
