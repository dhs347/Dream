...
- 算法
  - 过滤掉开头的$H$和结尾的$R$，调整$A$和$B$，计算新的$cnt_h,cnt_r$，特判特殊情况
  ...
  - 转移：考虑前$k$个元素中第一对(最左边)冲突的字母对，讨论$H$和$R$谁生还
    - $H$生还，$(i,j)\to(i,j-1)$
    - $R$生还，$(i,j)\to(i-1,j)$
...
- 正确性：由于边界的控制，转移是正确的
  - 考虑第一对冲突，不涉及当前第$i$个$H$
    - 若$R$生还，则没有任何问题
    - 若$H$生还，如果前面连续$H$个数超过了$B$，则方案非法，方案数为$0$
  - 考虑第一对冲突，涉及当前第$i$个$H$
    - 若$H$生还，则没有任何问题
    - 若$R$生还，违背了我们第$i$个$H$必须生还的要求，但是转移仍然是正确的：由于第$i$个$H$前紧靠着的是$R$，那么第$i-1$个$H$和第$i$个$H$之间至少有$1$个$R$，所以我们化归到的前驱状态$(i-1,j)$中的$j$是不可能不变的；
