...
需要变换的下标每个数都是$n$为的$k$进制数，那么下标空间为$[0,n^k)$

这相当于一种$k$维的每维长度为$n$的$fft$循环异或卷积变换($k$进制不进位加法)，按照高维$fft$的思路，应该按照某种维度的排列顺序(比如从高位到低位)，逐维正变换，然后每个位置独立做点积，最后再逐维逆变换(对应一种递归或者迭代过程)
