...
注：基础题，给定串$S$，求一个最长的子串$t\in S$，使得$t$可以被分成两段，每段是回文串；等价于求$max_i\{f_i+g_{i+1}\}$，令$f_i$表示以$i$下标结束的最长回文后缀长度，$g_i$表示 $i$下标开头的最长回文前缀的长度
...
注：基础题，求出本质不同回文串的数量分布（求每种回文串的个数），然后对每种快速 $check$ 一下，叠加答案即可；可以用 $manacher$ ，后缀自动机，回文自动机，字符串 $hash$ 多种做法实现。

11.http://www.lydsy.com/JudgeOnline/problem.php?id=2160

注：基础题，求回文子串数量和长度为奇数的回文子串长度之积模$19930726$
...
注：难题，$PAM$优化$dp$转移；给定两个字符串$A,B$，选择$A$的若干个不交的子串，分别翻转，使得$A=B$，求需要被翻转的最少子串个数或者指出无解。
