...
inline int mul(initializer_list<int> V) {
    int ret=1;
    for (auto t:V) ret=mul(ret,t);
    return ret;
}
...
int Merge(int x,int y,int p) {
    ...
    if (!x) {
        sb=add(sb,t[y].v);
        pd(y,(add(sa,p)-mul({2,sa,p}))%_p);
        return y;
    }
    if (!y) {
        sa=add(sa,t[x].v);
        pd(x,(add(sb,p)-mul({2,sb,p}))%_p);
        return x;
    }
...
void Query(int x,int l,int r) {
    ...
    if (l==r) {
        ++no;
        ans+=mul({no,tmp[l].x,t[x].v,t[x].v}),ans%=_p;
        return;
    }
    push_down(x);
    int mid=l+r>>1;
