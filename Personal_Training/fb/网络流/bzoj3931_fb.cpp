...
const int N = 505;
ll f[N][N], n, m, a[N * 20], b[N * 20], w[N * 20], x;
...
int main() {
	...
	rep(i, 1, m+1) {
		cin >> a[i] >> b[i] >> w[i];
		f[b[i]][a[i]] = f[a[i]][b[i]] = min(f[a[i]][b[i]], w[i]);
	}
	rep(k, 1, n+1)
		rep(i, 1, n+1)
		 	rep(j, 1, n+1) if (i != j) f[i][j] = min(f[i][k] + f[k][j], f[i][j]);
	G.ini(n * 2 + 10);
	rep(i, 1, n+1) {
		cin >> x;
		if (i == 1 || i == n) x = pw(60);
	...
	rep(i, 1, m+1) {
		if (f[1][a[i]] + w[i] == f[1][b[i]]) {
			G.link(a[i] + n, b[i], pw(60));
		}
		if (f[1][b[i]] + w[i] == f[1][a[i]]) {
