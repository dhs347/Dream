...
int T, n, m, o1, o2, d, a[N], b[N], va[N], vb[N], c;
string s1, s2, s;
...
int main() {
	...
	rep(cas, 0, T) {
		...
		rep(i, 0, d) {
			s1 = s;
			rep(j, 0, n) if (s1[j] == 'x') {
				if (pw(c) & i) s1[j] = 'b'; else s1[j] = 'a';
				c++;
			} 
			ts.init(n);
			rep(j, 0, m) {
				o1 = get(va[j], s1[a[j]] - 'a');
				o2 = get(vb[j], s1[b[j]] - 'a');
				if (o1 > 1) continue;
				if (o2 > 1) ts.add_set(a[j], o1 ^ 1);
				else ts.addedge(a[j], o1, b[j], o2);
			}
			if (ts.solve()) {
				rep(j, 0, n) cout << get2(s1[j], ts.mark[j]);
				cout << endl;
				return 0; 
			}
		}
